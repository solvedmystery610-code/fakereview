  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      setMessage(
        result?.otp_sent
          ? authView === "signup"
            ? "Signup OTP sent successfully. Please check your email."
            : "Login OTP sent successfully. Please check your email."
          : authView === "signup"
          ? "Signup OTP was generated, but it could not be delivered."
          : "Login OTP was generated, but it could not be delivered."
      );
      ...
      if (authView === "login") {
        localStorage.setItem(PENDING_AUTH_MODE_KEY, "login-otp");
      } else {
        localStorage.removeItem(PENDING_AUTH_MODE_KEY);
      }
    } catch (otpError) {
