...
MAX_TRAIN_ROWS = int(os.environ.get("FAKEREVIEW_TRAIN_ROWS", "8000"))
WORD_MAX_FEATURES = int(os.environ.get("FAKEREVIEW_WORD_MAX_FEATURES", "15000"))
CHAR_MAX_FEATURES = int(os.environ.get("FAKEREVIEW_CHAR_MAX_FEATURES", "15000"))
...
class TfidfLinearModel:
    ...
    def structural_feature_vector(self, text):
        ...
        return [
            # float(np.log1p(word_count)), # Removed due to bias
            float(unique_ratio),
            # float(np.log1p(sentence_count)), # Removed due to bias
            float(np.log1p(numeric_hits)),
            float(np.log1p(exclamations)),
            float(np.log1p(question_marks)),
            float(np.log1p(caps_words)),
            float(np.log1p(repeated_chars)),
            float(avg_token_length),
            float(long_token_ratio),
            float(np.log1p(repeated_bigrams)),
            float(tracked_ratio),
            float(average_idf),
    ...
    def _fit_meta_classifier(self, calib_rows):
        ...
        structural_scaled = self.scaler.fit_transform(structural)
        
        meta_features = np.column_stack([base_probabilities, structural_scaled])

        meta_classifier = LogisticRegression(max_iter=5000, C=0.01, solver="liblinear", penalty="l2", class_weight="balanced")
    ...
    def predict_batch_proba(self, texts):
        ...
        if self.meta_classifier is None or self.scaler is None or not texts:
            return base_probabilities
        structural = self._structural_feature_matrix(texts)
        structural_scaled = self.scaler.transform(structural)
        meta_features = np.column_stack([base_probabilities, structural_scaled])
    ...
    def _train(self):
        ...
        try:
            ...
            self.classifier = LogisticRegression(max_iter=MAX_ITER, C=LOGREG_C, solver="liblinear", class_weight="balanced")
            ...
            # Always use dynamic threshold selection to find the most balanced cut-off point
...
def analyze_review(review, rating, duplicate_info=None):
    ...
    # Structural bias: Longer, well-structured reviews are more likely to be genuine
    # Subtracting a small bias if the review is substantial and has natural negative sentiment
    ...
    if word_count < 4 and (signals.get("exclamations", 0) > 2 or signals.get("capsWords", 0) > 1):
        # Disabled aggressive short-review penalty to reduce false positives
        pass
    ...
    ai_phrases = [
        "overall, i", "highly recommend", "sleek and modern", "game changer", "game-changer",
        "exceeded my expectations", "enhances the", "huge plus", "reliable and easy",
        "audio quality is fantastic", "whole experience", "works like a charm",
        "would definitely buy again", "worth every penny", "cannot recommend this enough",
        "highly recommended", "look no further", "user friendly", "user-friendly", "extremely satisfied",
        "perfect addition to", "top notch", "top-notch", "five stars", "do not hesitate to buy",
        "you won't regret it", "easy to set up", "must-have", "must have",
        "absolutely love", "absolutely thrilled", "best purchase", "worth the money",
        "completely satisfied", "outstanding quality", "superb quality", "exceptional quality",
        "fast shipping", "quick delivery", "arrived quickly", "exactly as described",
        "incredibly impressed", "blown away", "friendly and helpful", "stands out from",
        "unbiased testimonial", "unbiased review", "honest review", "stunning performance",
        "impressed with the quality", "runs like a charm", "incredibly easy", "pleasantly surprised",
        "seamless experience", "impressed with the performance", "highly functional", "delivers on its promise",
        "delivers on promises", "does not disappoint", "well worth the", "definitely worth the",
        "top-tier", "top tier", "crystal clear", "outstanding service", "world class", "world-class"
    ]
    ai_hits = sum(1 for phrase in ai_phrases if phrase in text.lower())
    if ai_hits >= 2:
        probability += 0.38
        _add_reason(reasons, "Contains multiple phrases commonly generated by AI review templates")
    elif ai_hits == 1:
        probability += 0.20
        _add_reason(reasons, "Contains language matching common AI review template phrases")

    marketing_buzzwords = [
        "outstanding", "exceptional", "superb", "phenomenal", "flawless", 
        "incredible", "perfectly", "mind-blowing", "revolutionary", "breathtaking",
        "unmatched", "remarkable", "perfect", "stunning", "fantastic",
        "gorgeous", "brilliant", "stellar", "impeccable", "wonderful",
        "magnificent", "awesome", "first-class", "superior", "prime"
    ]
    buzz_hits = 0
    lower_text = text.lower()
    for word in marketing_buzzwords:
        if word in lower_text:
            negations = ["not", "isn't", "aren't", "wasn't", "weren't", "haven't", "hasn't", "hadn't", "don't", "doesn't", "didn't", "won't", "wouldn't", "can't", "cannot", "couldn't", "never", "hardly", "barely"]
            negated = False
            for neg in negations:
                pattern = rf"\b{neg}\s+(?:\w+\s+)?{word}\b"
    ...
    if buzz_hits >= 2:
        probability += 0.18
        _add_reason(reasons, "Uses multiple exaggerated promotional adjectives common in fabricated reviews")

    # Hook / Signature checks for AI Review Generators
    lower_text = text.lower()
    ai_starts = [
        "i recently purchased", "i have been using", "if you are looking for", 
        "i was skeptical at first", "i absolutely love", "i was pleasantly surprised",
        "just picked up", "i am absolutely thrilled", "i recently bought"
    ]
    ai_ends = [
        "highly recommended!", "highly recommended.", "highly recommend!", "highly recommend.",
        "worth every penny!", "worth every penny.", "five stars!", "five stars.",
        "definitely recommend!", "definitely recommend.", "must-have!", "must-have.",
        "you won't regret it!", "you won't regret it."
    ]
    start_hit = any(lower_text.startswith(start) for start in ai_starts)
    end_hit = any(lower_text.endswith(end) for end in ai_ends)
    if start_hit or end_hit:
        probability += 0.18
        _add_reason(reasons, "Starts or ends with a highly common AI-generated review signature")

    # Short simple template checks for mockup reviews
    simple_fakes = [
        "great product", "five stars", "excellent service", "highly recommended", 
        "exactly as described", "fast shipping", "very satisfied", "would recommend",
        "good product", "best purchase ever", "love it", "works great", "perfect"
    ]
    if word_count <= 6 and any(phrase in lower_text for phrase in simple_fakes):
        probability += 0.25
        _add_reason(reasons, "Matches common simple template-like reviews used in mockup generators")

    # Casing, grammar, and style difference checker (Manual vs AI signature)
    # AI review generators have perfect sentence casing, no spelling errors, and proper punctuation.
    # Manual reviews on the other hand frequently have:
    # 1. Missing capitalization of "i" -> e.g. "i love this"
    # 2. Words in all-lowercase at start of sentences -> e.g. "great product. works good."
    # 3. Missing spaces after punctuation -> e.g. "nice,good product."
    # 4. Common casual spelling errors -> e.g. "recieved", "dont", "cant", "wont", "greatt"
    has_manual_indicators = False
    if re.search(r"\bi\b", text):
        has_manual_indicators = True
    ...
    casual_typos = ["dont", "cant", "wont", "recieved", "awsome", "performence", "definetly", "recommendationo", "greatt", "niceee"]
    if any(typo in lower_text for typo in casual_typos):
        has_manual_indicators = True

    if has_manual_indicators:
        probability -= 0.05
    else:
        # Pristine grammar and formatting with at least some template indicators
        if ai_hits >= 1 or buzz_hits >= 1:
            probability += 0.12
            _add_reason(reasons, "Pristine grammar and capitalization with typical AI template markers")


    probability = clamp(probability, 0.01, 0.99)
    status = "Fake" if probability >= threshold else "Genuine"

    if status == "Genuine" and sentence_count >= 2 and word_count >= 18:
